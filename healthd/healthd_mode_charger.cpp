...
static void update_screen_state(charger* charger, int64_t now) {
	...
	if (batt_prop) {
            batt_anim->cur_level = getIntField(healthd_config->batteryCapacityPath);
...
static void process_key(charger* charger, int code, int64_t now) {
    ...
    LOGV("process key [%" PRId64 "] timestamp: [%" PRId64 "] down:%d pending:%d\n", now, key->timestamp, key->down,key->pending);
    if (code == KEY_POWER) {
        if (key->down) {
            //int64_t reboot_timeout = key->timestamp + POWER_ON_KEY_TIME;
...
static void init_shutdown_alarm(void)
{
    ...
    LOGW("pthread_create  alarm_thread_handler \n");
