...
static void check_fs(const char *blk_device, char *fs_type, char *target, int *fs_stat)
{
    ...
    if (is_extfs(fs_type)) {
        ...
        if (!ret) {
            ...
            {
                LINFO << "start format " <<blk_device;
                child = fork();
                if (child == 0) {
                    LWARNING << "fork to format " << blk_device << ",fileSystem: Fat16 ";
                    execl("/system/bin/newfs_msdos","/system/bin/newfs_msdos",
                          "-F","16","-c","4", blk_device, NULL);
                    exit(-1);
                }
                LWARNING << "wait for format "<< blk_device;
                while (waitpid(-1, &status, 0) != child) ;
                if (status==0)
                    LWARNING << "format " << blk_device <<"ok!";
            }
            else
            {
                LINFO <<"start format " << blk_device;
                child = fork();
                if (child == 0) {
                    LINFO << "fork to format " << blk_device;
                    execl("/system/bin/newfs_msdos","/system/bin/newfs_msdos",
                          "-F","32","-O","android","-c","8",blk_device, NULL);
                    exit(-1);
                }
                LINFO << "wait for format "<< blk_device;
                while (waitpid(-1, &status, 0) != child) ;
                if (status==0)
                    LWARNING << "format " << blk_device <<"ok!";
            }
